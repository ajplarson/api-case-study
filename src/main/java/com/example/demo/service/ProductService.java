...
public class ProductService {
  ...
  private String getProductResponseAsString(String productId) {
    String productEndpoint =
        format(
            "/v3/pdp/tcin/%s?excludes=taxonomy,price,promotion,bulk_ship,rating_and_review_reviews,rating_and_review_statistics,question_answer_statistics&key=candidate#_blank",
            productId);
    var responseSpec =
        client.get().uri(productEndpoint).accept(MediaType.APPLICATION_JSON).retrieve();
